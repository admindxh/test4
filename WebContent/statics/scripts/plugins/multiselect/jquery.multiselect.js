		...
		__multiSelect: function(o, callback) {
			...
			jQuery(this).each( function() {
				...
				if( select.next().hasClass("multi-select")||select.next().hasClass("multiSelect-cont") ){
					select.next()
						.addClass("multiSelect-cont")
						.find("input")
						.addClass("multiSelect").parent().find("i").addClass("multiSelect-span") ;
				}else{
					var html = '<span class="multiSelect-cont  input-append"><input type="text" readonly="readonly" class="multiSelect include-span input-span-uneditable" value="" title="" style="width:' + (width-26) + 'px;" />';
				...
				_multiSelect.parent().find(".add-on")
					...
					function optionsShow() {
						...
						_multiSelectOptions.width(me.parent().width()-2).show().position(pos);
