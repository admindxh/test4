/*
 * My97 DatePicker 4.7 Beta6 License: http://www.my97.net/dp/license.asp
 */
var $dp, WdatePicker;
(function() {
	var _ = {
	...
	function L() {
		...
		return jQuery.utils.scriptPath("plugin")+"calendar/" ; ;
		//return Global.contextPath+"/widgets/core/calendar/" ;
	}
	function F(F) {
		/*var E, C;
		...
		_.$dpPath = jQuery.utils.scriptPath("plugin")+"calendar/" ; ;
		_.$basePath = Config.contextPath ;
	...
	function getUIBasePath(){
			var path = jQuery.utils.scriptPath("jqueryui.css") ;
			/*
			var __basepath = '' ;
			var __scripts = document.getElementsByTagName('link');
			for (var i = 0; i < __scripts.length; i++) {
				var script = __scripts[i];
				var _ = script.href;
				if (_.indexOf('ui.css') != -1) {
					__basepath = _ ;
				}
			}*/
			return path  ;
	...
	function K(H, $) {
		...
		var _oldTheme = $dp.theme ;
					
		//var theme,themeEl = document.getElementById('themesLink') ;
		//if(themeEl)theme = themeEl.href ;
		
		var theme = getUIBasePath() ;
		...
		if ( (!$dp.dd
				...
				|| ($dp.lang && $dp.realLang && $dp.realLang.name != $dp.lang
						&& $dp.getLangIndex && $dp.getLangIndex($dp.lang) >= 0) ) || needReload
		) {
			...
			var B = "<iframe style=\"width:1px;height:1px\" src=\""
					+ _.$dpPath
					+ "My97DatePicker.htm?theme="+encodeURIComponent($dp.theme)+"\" frameborder=\"0\" border=\"0\" scrolling=\"no\"></iframe>";
			if ($dp.eCont) {
		...
		function C() {
			...
			jQuery($dp.dd).position(pos);
		}
	}
})();

//add jquery instance method by chenjc

(function($) {
	  

  $.fn.calendar = function(json4Options){
  	 if($(this).get(0).tagName == 'INPUT'){
  	 	 $(this).addClass("Wdate") ;
  	 	 $(this).get(0).onclick = function(){ WdatePicker(json4Options);} ;
  	 }else{
  	 	json4Options.eCont = $(this).get(0).id;
  	 	WdatePicker(json4Options);
  	 }
  };
  
  $.calendarInit = function(jqueryObj,json4Options){
      jqueryObj.calendar(json4Options);
  };

  
   $.uiwidget.register("calendar",function(selector){
		selector.each(function(){
	  	 var options = $(this).attr( $.uiwidget.options )||"{}";
		 eval(" var jsonOptions = "+options) ;
		 $(this).calendar( jsonOptions ) ;
	  }) ;
	}) ;
