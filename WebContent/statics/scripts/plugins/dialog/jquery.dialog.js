(function(){
	...
	$.fn.dialogResize = function(opts){
		...
		if(!dialog){
			if( window.parent.$("iframe[name='"+window.name+"']").parents(".ui-dialog-wrapper:first")[0]){
				var pos = {} ;
				try{
			...
			$(window).keydown(function(e){
				...
				if(kNo==27){
					...
					if(dialog.dialog){
						...
						dialogWin.$ && dialogWin.$.dialog_ESCClose() ;
					}
				};
			});
		}
	...
	function resizeDialog(opts){
        ...
        if (opts.iframe) {
            ...
            container.find(".dialog-content>iframe").height(Math.min(height, _height));
   
            if (isIE6() && isScrollX) container.find(".dialog-content>iframe").width(width - 14);
        }
