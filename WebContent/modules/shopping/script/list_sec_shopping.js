...
function createShopping() {
	...
	var url = '~/commodity/createCommodity.do';
...
//修改商品信息
function updateCommodity(grid, rowData, keyData) {
	if (keyData) {
...
//删除商品
function deleteCommodity(grid, rowData, keyData) {
	if (keyData) {
		var commodityName = rowData['commodityName'];
		$.messageBox.confirm({
					message : "您确定要删除\"" + commodityName + "\"商品吗？",
					callback : function(result) {
						if (result) {
							$.dataservice("spring:commodityService.deleteCommodity", {
								     commodityId : keyData
									}, function() {
										shopping_grid.refresh();
									});
	...
	$.messageBox.confirm({
				...
				callback : function(result) {
					...
					for (var i = 0; i < ids.length; i++) {
						vals.push(shopping_grid.getCellValue("" + ids[i], 'commodityId'));
					}
					$.dataservice("spring:commodityService.deleteAllCommodity", {
						commodityIds : vals.join(",")
							}, function(response) {
								shopping_grid.refresh();
							});
				}
