...
function createProfile() {
	var url = '~/profile/createProfile.do'
...
function updateProfile(grid, rowData, keyData) {
	if (keyData) {
		var url = '~/profile/editProfile.do?profileId=' + keyData;
		$.open(url, 810, null, {}, function() {
					if ($.dialogReturnValue()) {
						grid.refresh();
					}
...
function copyProfile(grid, rowData, keyData) {
	if (keyData) {
		var url = '~/profile/copyProfile.do?profileId=' + keyData;
		$.open(url, 810, null, {}, function() {
					if ($.dialogReturnValue()) {
						grid.refresh();
					}
...
function createProfileParameter() {
	...
	var url = '~/profile/createProfileParameter.do?profileId='
...
function updateProfileParameter(grid, rowData, keyData) {
	if (keyData) {
		var url = '~/profile/editProfileParameter.do?profileParameterId='
				+ keyData;
		$.open(url, 500, null, {}, function() {
					if ($.dialogReturnValue()) {
						grid.refresh();
